package com.example.homeworkaudioplayer;
...
public class MainActivity extends AppCompatActivity implements MediaPlayer.OnPreparedListener,MediaPlayer.OnCompletionListener{
        ...
        try {
            switch (view.getId()) {
                ...
                case R.id.btnRAW:
                    ...
                    toast.show(); // демонстрация тоста на экране
                    mediaPlayer = MediaPlayer.create(this, R.raw.caitlin_de_ville); // создание дорожки с имеющимся аудио-файлом
                    mediaPlayer.start(); // старт данной дорожки
                    nameAudio = "Кейтлин Де Виль - Скрипка"; // инициализация названия контента
                    break;
                case R.id.btnSD:
        ...
        switch (view.getId()){
            case R.id.btnResume:
                if (!mediaPlayer.isPlaying()){
                    mediaPlayer.start(); // метод возобновления проигрования
                }
                break;
            case R.id.btnPause:
                if (mediaPlayer.isPlaying()){
            ...
            case R.id.btnForward:
                mediaPlayer.seekTo(mediaPlayer.getCurrentPosition()+5000); // переход к определенной позиции трека
                break;
            case R.id.btnBack:
                mediaPlayer.seekTo(mediaPlayer.getCurrentPosition()-5000); // переход к определенной позиции трека
                break;
    ...
    public void onPrepared(MediaPlayer mediaPlayer) { // метод подготовки дополнительного потока
        ...
        toast.show(); // демонстрация на экране
    }
